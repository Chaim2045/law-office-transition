  ...
  async loadBlocksFromFirebase() {
    ...
    try {
      ...
      if (firebaseData) {
        ...
        // שלב 2: עדכון תוכן בלוקים קיימים + טעינת בלוקים חדשים
        Object.keys(firebaseData).forEach((blockId) => {
          ...
          if (blockId.startsWith('block_') && !blockId.startsWith('block_meta')) {
            ...
            if (block && block.content) {
              // Block exists in DOM - update content
              ...
              // Block doesn't exist in DOM - it was created by another user
              // We need metadata to recreate it, but for now just log it
              console.log(`📝 [Load] Block ${blockId} exists in Firebase but not in DOM - needs metadata to recreate`);
            }
          }
        });
      } else {
